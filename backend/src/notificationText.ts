...
- "안녕"으로 매번 시작하지 마.
- 사용자가 보고 있는 게 "영상"이라고 단정하지 마 — 유튜브·인스타 영상일 수도 있지만 글, 사진, 채팅 등 영상이 아닐 수도 있어. "무슨 영상 보고 있어요?"처럼 형식을 단정하는 표현은 절대 쓰지 말고, "지금 뭐 보고 있어요?"처럼 형식을 특정하지 않는 중립적인 표현만 써.
...
[출력 규칙]
문장 1개, 15자~${MAX_LENGTH}자 사이. 이모티콘·이모지 쓰지 마. 따옴표나 설명 없이 문자 내용 그 자체만 출력해.`;
}
