...
${openingStyle}
예시 문장은 "이런 식으로 말을 거는 구조"를 보여주는 참고용일 뿐이니 그대로 베끼지 말고, 단어 선택과 문장 구조를 새롭게 바꿔서 써. 예시 문장의 말투(존댓말/반말)는 무시하고, 실제 말투는 반드시 아래 [말투] 섹션만 따를 것 — 문장 구조와 말투는 서로 다른 지시이니 섞지 말 것. 전화 거는 이유를 그럴듯하게 지어내지 마("네 생각나서" 같은 핑계 금지) — 그냥 담백하게 말 걸면 돼.
